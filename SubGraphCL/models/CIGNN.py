...
from .CIGNN_modules.memory import Memory
from .CIGNN_modules.buffer import Triad_Buffer

from torch.distributions import Bernoulli
from torch.autograd import Variable
from torch.distributions import Normal

from .CIGNN_utils import IB, PGen
import random
from .general_utils import TimeEncode
...
class CIGNN(nn.Module):
    def __init__(
        ...
        self.memory = Memory(
            ...
            emb_dim=self.node_emb_dim,
            device=device,
            features=None,
        )
        self.triad_buffer = Triad_Buffer(
            n_edges=len(label_src),
            n_task=n_task,
            per_class=per_class,
            label_src=label_src,
            label_dst=label_dst,
            ...
            memory=self.memory.emb,
            n_vol=mem_size,
            radius=radius,
            gamma=gamma,
        )
        if self.dis_IB:
            self.IB = nn.ModuleList(
                [
                    IB(
                        shape_x=self.node_emb_dim,
                        shape_z=self.node_emb_dim,
                        shape_y=per_class,
                        label_src=label_src,
                        label_dst=label_dst,
                        per_class=per_class,
        ...
        if self.use_time == 0:
            self.W_q = nn.Parameter(
                torch.zeros((self.node_emb_dim, self.node_emb_dim)).to(self.device)
            )
            ...
            self.W_k = nn.Parameter(
                torch.zeros((self.node_emb_dim + self.use_time, self.node_emb_dim)).to(
                    self.device
                )
            )
        ...
        if self.npr:
            self.W_uc = nn.Parameter(
                torch.zeros(
                    (self.node_emb_dim + self.node_emb_dim, self.node_emb_dim)
                ).to(self.device)
            )
        else:
            self.W_uc = nn.Parameter(
                torch.zeros(
                    (
                        self.node_emb_dim + self.node_emb_dim * 2 + self.edge_emb_dim,
                        self.node_emb_dim,
                    )
                ).to(self.device)
            )  # update_center
        ...
        if self.select != "none":
            ...
            self.W_p = nn.Parameter(
                torch.zeros(
                    (2 * self.node_emb_dim + self.edge_emb_dim, self.node_emb_dim)
                ).to(self.device)
            )  # propagate
        ...
        if self.use_feature == "fg":
            self.W_c1 = nn.Parameter(
                torch.zeros((self.node_emb_dim + self.node_init_dim, 128)).to(
                    self.device
                )
            )
            ...
            self.W_c1 = nn.Parameter(
                torch.zeros((self.node_emb_dim, 128)).to(self.device)
            )
        elif self.use_feature == "f":
            self.W_c1 = nn.Parameter(
                torch.zeros((self.node_init_dim, 256)).to(self.device)
            )
        ...
        if self.head == "single":
            ...
            if not self.sc:
                self.W_c2_list = [
                    nn.Parameter(torch.zeros((128, self.per_class))).to(self.device)
                    for i in range(self.n_task)
                ]
            else:
                self.W_c2_list = [
                    nn.Parameter(torch.zeros((self.node_init_dim + 128, 128))).to(
                        self.device
                    )
                    for i in range(self.n_task)
                ]
        ...
        if self.use_time > 0:
            self.te = TimeEncode(expand_dim=use_time, device=self.device)

        if self.feature_iter:
            self.W_f = nn.Linear(
                self.node_emb_dim + self.node_init_dim, self.node_emb_dim
            ).to(self.device)
        ...
        # select layers
        self.W_1 = nn.Linear(
            self.node_emb_dim + self.node_emb_dim * 2 + edge_emb_dim, self.node_emb_dim
        ).to(self.device)
    ...
    def forward(
        ...
        if self.head == "single":
            ...
            if tmp_Wc.shape[1] < (task_id + 1) * self.per_class:
                ...
                self.W_c2 = nn.Parameter(
                    torch.zeros(tmp_Wc.shape[0], (task_id + 1) * self.per_class).to(
                        self.device
                    )
                )
                nn.init.xavier_normal_(self.W_c2)
        ...
        if self.model == "OTGNet":
            ...
            for i, idxs in enumerate(
                [src_idxs, dst_idxs]
            ):  # traverse src and dst in turns
                neighbors, _, n_times = self.neighbor_finder.get_temporal_neighbor(
                ...
                if self.feature_iter:
                    neighbor_emb = self.W_f(
                        torch.cat(
                            (neighbor_emb, self.node_feature[neighbors.flatten()]),
                            dim=1,
                        )
                    )
                if self.use_IB:
                    if mask_node is not None:
                        ...
                        test_node = list(
                            mask_node
                            & set(torch.tensor(idxs)[cur_mask.cpu()].numpy().tolist())
                        )
                        ...
                        cnt_num = 0
                        if self.uml:
                            src_lg2, dst_lg2 = self.pgen(
                                self.node_feature,
                                self.memory.emb.data,
                                src_idxs,
                                dst_idxs,
                                self.label_src[edge_idxs],
                                self.label_dst[edge_idxs],
                                eval_task,
                    ...
                    if self.use_IB and torch.sum(n_mp) > 0:
                        ...
                        if not self.dis_IB:
                            n_z = self.IB.forward(
                                x=neighbor_emb.data[n_mp],
                                y=n_label.flatten()[n_mp],
                                task_id=task_id,
                            )
                            ...
                            task_mask_n = {}
                            for c in range(
                                0, (task_id + 1) * self.per_class, self.per_class
                            ):
                                ...
                                if torch.sum(task_mask_n[t]) > 0:
                                    n_z = self.IB[t].forward(
                                        x=neighbor_emb.data[task_mask_n[t]],
                                        y=n_label.flatten()[task_mask_n[t]],
                                        task_id=task_id,
                                        cur_id=t,
                                    )
                                    neighbor_emb[task_mask_n[t]] = n_z

                neighbor_emb = neighbor_emb.view(
                    bs, self.n_neighbors, self.node_emb_dim
                )

                if self.use_time > 0:
                    n_times = torch.tensor(timestamps).view(
                        len(timestamps), 1
                    ) - torch.tensor(n_times)
                    n_times[n_times < 0] = 0
                ...
                if self.feature_iter:
                    center_emb = self.W_f(
                        torch.cat((center_emb, self.node_feature[idxs]), dim=1)
                    )
                h_c = (
                    torch.matmul(center_emb, self.W_q)
                    .unsqueeze(dim=1)
                    .repeat(1, self.n_neighbors, 1)
                )
                ...
                att = (
                    att.softmax(dim=1).unsqueeze(dim=2).repeat(1, 1, self.node_emb_dim)
                )
                h = h_n * att
                ...
                if self.use_IB and torch.sum(mp) > 0:
                    if not self.dis_IB:
                        ...
                        for c in range(
                            ...
                            if torch.sum(task_mask_n[t]) > 0:
                                z = self.IB[t].forward(
                                    x=(message[i]).data[task_mask_n[t]],
                                    y=(self.node_label[idxs])[task_mask_n[t]],
                                    task_id=task_id,
                                    cur_id=t,
                                )
            ...
            if self.npr:
                to_updated_src = torch.matmul(
                    torch.cat((self.memory.emb[src_idxs], message[0]), dim=1), self.W_uc
                ).tanh()
                to_updated_dst = torch.matmul(
                    torch.cat((self.memory.emb[dst_idxs], message[1]), dim=1), self.W_uc
                ).tanh()
            else:
                ...
                to_updated_src = torch.matmul(
                    torch.cat((self.memory.emb[src_idxs], h), dim=1), self.W_uc
                ).tanh()
                to_updated_dst = torch.matmul(
                    torch.cat((self.memory.emb[dst_idxs], h), dim=1), self.W_uc
                ).tanh()
        ...
        if self.select != "none":
            for idxs in [src_idxs, dst_idxs]:
                ...
                bs = neighbors.shape[0]
                neighbor_emb = self.memory.emb[neighbors.flatten()].view(
                    bs, self.n_neighbors, self.node_emb_dim
                )
                ...
                if self.n_mc > 0:
                    h1 = h1 * self.confidence[i].unsqueeze(-1).unsqueeze(-1).repeat(
                        1, h1.shape[1], h1.shape[2]
                    )
                ...
                changed_emb = torch.matmul(
                    torch.cat(
                        (
                            self.memory.emb[neighbors.flatten()],
                            changed_emb.flatten().view(-1, self.node_emb_dim),
                        ),
                        dim=1,
                    ),
                    self.W_un,
                ...
                mask = (
                    ...
                    .repeat(1, 1, self.node_emb_dim)
                    .flatten()
                    .view(-1, self.node_emb_dim)
                )
                self.memory.emb[neighbors.flatten()] = (
                    mask * changed_emb
                    + (1 - mask) * self.memory.emb[neighbors.flatten()]
                )

        # compute loss
        if self.model == "OTGNet":
            if self.use_feature == "fg":
                pre_src_logits = torch.matmul(
                    torch.cat(
                        (self.memory.emb[src_idxs], self.node_feature[src_idxs]), dim=1
                    ),
                    self.W_c1,
                )
                pre_dst_logits = torch.matmul(
                    torch.cat(
                        (self.memory.emb[dst_idxs], self.node_feature[dst_idxs]), dim=1
                    ),
                    self.W_c1,
                )
            elif self.use_feature == "g":
            ...
            if self.uml and ch != "inf":
                src_logits2, dst_logits2, loss_s2, loss_d2 = self.pgen(
                    self.node_feature,
                    self.memory.emb.data,
                    src_idxs,
                    dst_idxs,
        ...
        if self.head == "single":
            ...
            src_logits = torch.zeros((len(cur_label_src), self.per_class)).to(
                self.device
            )
            dst_logits = torch.zeros((len(cur_label_dst), self.per_class)).to(
                self.device
            )
            task_mask_src = {}
            task_mask_dst = {}
            for c in range(0, (task_id + 1) * self.per_class, self.per_class):
                ...
                if not self.sc:
                    src_logits[task_mask_src[t]] = torch.matmul(
                        pre_src_logits[task_mask_src[t]], self.W_c2_list[t]
                    )
                    dst_logits[task_mask_dst[t]] = torch.matmul(
                        pre_dst_logits[task_mask_dst[t]], self.W_c2_list[t]
                    )
                else:
                    src_logits_p = torch.matmul(
                        torch.cat(
                            (
                                pre_src_logits[task_mask_src[t]],
                                self.node_feature[src_idxs][task_mask_src[t]],
                            ),
                            dim=1,
                        ),
                        self.W_c2_list[t],
                    )
                    dst_logits_p = torch.matmul(
                        torch.cat(
                            (
                                pre_dst_logits[task_mask_dst[t]],
                                self.node_feature[dst_idxs][task_mask_dst[t]],
                            ),
                            dim=1,
                        ),
                        self.W_c2_list[t],
                    ...
                    src_logits[task_mask_src[t]] = torch.matmul(
                        src_logits_p, self.W_c3_list[t]
                    )
                    dst_logits[task_mask_dst[t]] = torch.matmul(
                        dst_logits_p, self.W_c3_list[t]
                    )

                loss_s[task_mask_src[t]] = self.criterion_list(
                    src_logits[task_mask_src[t]],
                    cur_label_src[task_mask_src[t]] - t * self.per_class,
                )
                loss_d[task_mask_dst[t]] = self.criterion_list(
                    dst_logits[task_mask_dst[t]],
                    cur_label_dst[task_mask_dst[t]] - t * self.per_class,
                )
        ...
        if self.class_balance and oldsize > 0 and ch == "train":
            ...
            for c in range(task_id * self.per_class, (task_id + 1) * self.per_class):
                ...
                loss_s[class_mask_src[c]] = (
                    (newsize - class_num_src[c]) / newsize * loss_s[class_mask_src[c]]
                )
                loss_d[class_mask_dst[c]] = (
                    (newsize - class_num_dst[c]) / newsize * loss_d[class_mask_dst[c]]
                )
        ...
        if task_id > 0 and oldsize > 0 and ch == "train":
            ...
            ratio = 0.5
            loss = (1 - ratio) * loss_s[:newsize].mean() + ratio * loss_s[
                newsize:
            ].mean()
            loss += (1 - ratio) * loss_d[:newsize].mean() + ratio * loss_d[
                newsize:
            ].mean()
        else:
    ...
    def get_pattern_loss(self):
        pos_score = torch.sum(
            self.memory.emb[self.triad_buffer.pos_src]
            * self.memory.emb[self.triad_buffer.pos_dst],
            dim=1,
        )
        neg_score = torch.sum(
            self.memory.emb[self.triad_buffer.neg_src]
            * self.memory.emb[self.triad_buffer.neg_dst],
            dim=1,
        )
    ...
    def compute_score(self, src_idxs, dst_idxs, neg_idxs):
        pos_score = torch.sum(
            self.memory.emb[src_idxs] * self.memory.emb[dst_idxs], dim=1
        )
        neg_score = torch.sum(
            self.memory.emb[src_idxs] * self.memory.emb[neg_idxs], dim=1
        )
