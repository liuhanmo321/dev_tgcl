...
from models.TGN_utils import MergeLayer
from models.general_utils import TimeEncode
from models.TGN_modules.memory import Memory
from models.TGN_modules.message_aggregator import get_message_aggregator
from models.TGN_modules.message_function import get_message_function
from models.TGN_modules.memory_updater import get_memory_updater
from models.TGN_modules.embedding_module import get_embedding_module
...
class TGN(torch.nn.Module):
    ...
    def get_embeddings(
        ...
        n_samples = len(source_nodes)
        nodes = np.concatenate([source_nodes, destination_nodes, negative_nodes]) if negative_nodes != None \
                else np.concatenate([source_nodes, destination_nodes])
        positives = np.concatenate([source_nodes, destination_nodes])
        timestamps = np.concatenate([edge_times, edge_times, edge_times])
        ...
        if self.use_memory:
            ...
            if negative_nodes is not None:
                ...
                time_diffs = torch.cat(
                    [source_time_diffs, destination_time_diffs], dim=0
                )
