...
class EarlyStopMonitor(object):
    ...
    def early_stop_check(
        ...
        if self.last_best is None:
            ...
            model_path, mem_path, IB_path, PGen_path = get_checkpoint_path(
                model, time_now, task, train_PGen_backup == None
            )
            cur_model.set_features(None, None)
        ...
        elif (curr_val - self.last_best) / (np.abs(self.last_best) + 1e-10) > self.tolerance:
            self.last_best = curr_val
            self.num_round = 0
            self.best_epoch = self.epoch_count
            model_path, mem_path, IB_path, PGen_path = get_checkpoint_path(
            ...
            cur_model.set_features(None, None)
            # cur_model.model.base_model.node_raw_features = None
            # cur_model.model.base_model.edge_raw_features = None
            torch.save(cur_model, model_path)
            torch.save(train_memory_backup, mem_path)
            if train_IB_backup is not None:
                torch.save(train_IB_backup, IB_path)
...
def single_get_neighbor_sampler(data, sample_neighbor_strategy: str = 'recent', mask = None, time_scaling_factor: float = 0.0, seed: int = None):
    """
    ...
    for src_node_id, dst_node_id, edge_id, node_interact_time in zip(data.src, data.dst, data.edge_idxs, data.timestamps):
        if (mask is None) or (mask is not None and dst_node_id not in mask.induct_nodes):
            adj_list[src_node_id].append((dst_node_id, edge_id, node_interact_time))
        # if (mask is None) or (mask is not None and src_node_id not in mask.induct_nodes):
...
def get_neighbor_sampler(data, sample_neighbor_strategy: str = 'recent', mask = None, time_scaling_factor: float = 0.0, seed: int = None):
    """
    get neighbor sampler
    :param data: Data
    :param sample_neighbor_strategy: str, how to sample historical neighbors, 'uniform', 'recent', or 'time_interval_aware''
...
#     for src, dst, edge_idx, timestamp in zip(
#         data.src, data.dst, data.edge_idxs, data.timestamps
#     ):
...
#             print(src_idx)
#             print(len(self.node_to_edge_time_stamps))
#         i = np.searchsorted(self.node_to_edge_time_stamps[src_idx], end_time)
#         j = np.searchsorted(self.node_to_edge_time_stamps[src_idx], start_time)
#         return (
#             self.node_to_neighbors[src_idx][j:i],
#             self.node_to_edge_idxs[src_idx][j:i],
#             self.node_to_edge_time_stamps[src_idx][j:i],
#         )
    
#     # def find_after(self, src_idx, cut_time):
#     #     """
#     #     Extracts all the interactions happening after cut_time
#     #     for user src_idx in the overall interaction graph.
#     #     The returned interactions are sorted by time.
...
#     #     """
#     #     i = np.searchsorted(self.node_to_edge_time_stamps[src_idx], cut_time)
#     #     future_idx = max(i+1, len(self.node_to_edge_time_stamps[src_idx])-1)
#     #     return (
#     #         self.node_to_neighbors[src_idx][future_idx:],
#     #         self.node_to_edge_idxs[src_idx][future_idx:],
#     #         self.node_to_edge_time_stamps[src_idx][future_idx:],
#     #     )

#     def get_temporal_neighbor(self, src_nodes, timestamps, n_neighbors=20, find_future=False):
#         """
...
#             )
#             # else:
#             #     before_src_neighbors, before_src_edge_idxs, before_src_edge_times = self.find_before(
#             #         src_node, timestamp
#             #     )

#             #     after_src_neighbors, after_src_edge_idxs, after_src_edge_times = self.find_after(
#             #         src_node, timestamp
#             #     )

#             #     src_neighbors = np.concatenate((before_src_neighbors, after_src_neighbors))
#             #     src_edge_idxs = np.concatenate((before_src_edge_idxs, after_src_edge_idxs))
#             #     src_edge_times = np.concatenate((before_src_edge_times, after_src_edge_times))
...
#                     # else:
#                     #     half_neighbors = int(n_neighbors/2)
#                     #     source_edge_times = np.concatenate([before_src_edge_times[-half_neighbors:], after_src_edge_times[:half_neighbors]])
#                     #     source_neighbors = np.concatenate([before_src_neighbors[-half_neighbors:], after_src_neighbors[:half_neighbors]])
#                     #     source_edge_idxs = np.concatenate([before_src_edge_idxs[-half_neighbors:], after_src_edge_idxs[:half_neighbors]])
...
#     def get_temporal_neighbor_mask(self, src_node_nghs, start_times, end_times, n_neighbors=20):
#         tmp_n_neighbors = n_neighbors if n_neighbors > 0 else 1
#         # All interactions described in these matrices are sorted in each row by time
#         link_mask = np.zeros((len(src_node_nghs), tmp_n_neighbors, tmp_n_neighbors)).astype(np.float32)
#         # each entry in position (i,j) represent the id of the item
#         # targeted by user src_idx_l[i] with an interaction happening
#         # before cut_time_l[i]
#         for k, src_node_ngh, start_time, end_time in enumerate(zip(src_node_nghs, start_times, end_times)):
#             ngh_ngh_between = []
#             for ngh_i in range(tmp_n_neighbors):
#                 ngh_nghs, _, _ = find_between(src_node_ngh[ngh_i], start_time, end_time)
#                 ngh_ngh_between.append(ngh_nghs)
            
#             for ngh_i in range(tmp_n_neighbors):
#                 for ngh_j in range(ngh_i + 1):
#                     if src_node_ngh[ngh_j] in ngh_ngh_between[ngh_i]:
#                         link_mask[k, ngh_i, ngh_j] = 1
#                     elif src_node_ngh[ngh_j] == src_node_ngh[ngh_i]:
#                         link_mask[k, ngh_i, ngh_j] = 0
#                     else:
#                         link_mask[k, ngh_i, ngh_j] = -1
