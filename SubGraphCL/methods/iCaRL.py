...
class iCaRL(nn.Module):
    ...
    def begin_task(self, args, train_data, task):
        ...
        for c in range(task * self.args.num_class_per_dataset):
            ...
            mean_emb = torch.mean(total_emb_bank, dim=0)

            selected_emb = torch.zeros_like(total_emb_bank[:args.memory_size]).to(self.args.device)
            # selected_mp = torch.tensor([False for i in range(len(src_emb_bank))]).to(self.args.device)
            for i in range(args.memory_size):
                selected_sum_emb = torch.sum(selected_emb[:i+1], dim=0)
                additional_dist = (total_emb_bank+selected_sum_emb)/(i + 2)
                dist = (additional_dist - mean_emb).norm(2, dim=1)

                min_idx = torch.argmin(dist)
                # print(min_idx)
                selected_emb[i] = total_emb_bank[min_idx]
                total_emb_bank[min_idx] = float('inf')
                
                abs_idx = min_idx - len(src_emb_bank[task_mp_src]) if min_idx >= len(src_emb_bank[task_mp_src]) else min_idx
                if min_idx >= len(src_emb_bank[task_mp_src]):
                    src_collect.append(past_train_data.src[task_mp_dst][abs_idx])
                    dst_collect.append(past_train_data.dst[task_mp_dst][abs_idx])
                    timestamp_collect.append(past_train_data.timestamps[task_mp_dst][abs_idx])
                    edge_collect.append(past_train_data.edge_idxs[task_mp_dst][abs_idx])
                    label_src_collect.append(past_train_data.labels_src[task_mp_dst][abs_idx])
                    label_dst_collect.append(past_train_data.labels_dst[task_mp_dst][abs_idx])
                else:
                    src_collect.append(past_train_data.src[task_mp_src][abs_idx])
                    dst_collect.append(past_train_data.dst[task_mp_src][abs_idx])
                    timestamp_collect.append(past_train_data.timestamps[task_mp_src][abs_idx])
                    edge_collect.append(past_train_data.edge_idxs[task_mp_src][abs_idx])
                    label_src_collect.append(past_train_data.labels_src[task_mp_src][abs_idx])
                    label_dst_collect.append(past_train_data.labels_dst[task_mp_src][abs_idx])
            
            if cur_memory == None:
                cur_memory = Data(src_collect, dst_collect, \
                                    timestamp_collect, edge_collect, \
                                    label_src_collect, label_dst_collect)
            else:
                cur_memory.add_data(Data(src_collect, dst_collect, \
                                    timestamp_collect, edge_collect, \
                                    label_src_collect, label_dst_collect))
...
class Data:
    def __init__(
        ...
        self.labels_src = np.array(labels_src)
        self.labels_dst = np.array(labels_dst)
